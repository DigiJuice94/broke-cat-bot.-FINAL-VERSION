...
export function buyPost({mode='PAPER',symbol,sizeUsd,score,marketCap,risk,cash,walletLabel='Balance'}){const bundle=Number.isFinite(risk?.bundlePct)?`${risk.bundlePct.toFixed(1)}% ${String(risk.bundleRisk).toUpperCase()} (Solana Tracker)`:`${String(risk?.bundleRisk||'unknown').toUpperCase()} (${risk?.bundleStatus||'NO_DATA'})`;return `🐱 BROKE CAT ${mode} BUY\n$${symbol} | ~$${sizeUsd.toFixed(2)}\nScore ${score}/100 | MC $${Math.round(marketCap).toLocaleString()}\nBundle ${bundle}\n${walletLabel} ~$${cash.toFixed(2)}\n\nAutomated experiment. High risk.`}
