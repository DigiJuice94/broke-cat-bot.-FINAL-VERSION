...
const server=http.createServer(async(req,res)=>{
  if(req.url==='/health'){
    ...
    const payload=isLive?{ok:true,version:'v8.7',mode:'LIVE',wallet:walletAddress(),sol:lastWallet?.sol??null,solUsd:lastWallet?.solUsd??null,solValueUsd:lastWallet?.solValueUsd??null,dailyPnl:state.dailyPnl,realizedPnl:state.realizedPnl,hasPosition:Boolean(state.position),openPositions:lastPositions.length,positions:lastPositions,xPosting:xReady(),scans,lastScanAt,lastError,stateFile:liveStateFilePath()}:{ok:true,version:'v8.7',mode:'paper',cash:state.cash,equity:equityUsd(state),dailyPnl:state.dailyPnl,hasPosition:Boolean(state.position),openPositions:lastPositions.length,positions:lastPositions,xPosting:xReady(),scans,lastScanAt,lastError,stateFile:stateFilePath()};
    res.writeHead(200,{'content-type':'application/json'});res.end(JSON.stringify(payload));return;
  }
  res.writeHead(200,{'content-type':'text/plain'});res.end(`Broke Cat Bot V8.7 ${isLive?'LIVE MODE':'PAPER MODE'} 🐱`);
});
...
if(isLive){lastWallet=await walletSnapshot();const recovery=await reconcileLivePosition(state).catch(e=>({recovered:false,reason:e.message}));lastPositions=await scanWalletPositions(state).catch(()=>[]);if(recovery.recovered)await alert(`🐱 RECOVERED POSITION ${recovery.position.symbol} | mint ${recovery.position.tokenAddress} | raw ${recovery.position.tokenAmountRaw} | monitoring exits again`);else if(lastPositions.length)await alert(`🐱 WALLET POSITIONS FOUND: ${lastPositions.map(p=>`${p.symbol} ~$${p.currentValueUsd.toFixed(2)}`).join(' | ')} | recovery: ${recovery.reason}`);await alert(`🐱 Broke Cat Bot V8.7 STARTED | 🔴 LIVE MONEY | wallet ${walletAddress()} | SOL ${lastWallet.sol.toFixed(6)} (~$${lastWallet.solValueUsd.toFixed(2)}) | AUTO SIZE ${config.autoPositionSizing?'ON':'OFF'} | allowed wallet ${config.autoSizeMinWalletPct}-${config.autoSizeMaxWalletPct}% | network reserve ${config.minSolReserve} SOL | daily stop -$${config.maxDailyLoss.toFixed(2)}`)}
else await alert(`🐱 Broke Cat Bot V8.7 started | PAPER MODE | bankroll $${state.cash.toFixed(2)} | min score ${config.minScore} | Helius ${config.heliusApiKey?'ON':'OFF'} | SolanaTracker bundle scanner ${config.solanaTrackerApiKey?'ON':'OFF'} | X ${xReady()?'ON':'OFF'}`);
